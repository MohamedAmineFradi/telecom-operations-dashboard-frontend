  ...
  if (role === 'director') {
    ...
    const attentionPoints = [
      ...(congestionMetrics.critical > 0 ? [{
        id: 1,
        title: `${congestionMetrics.critical} zones critiques détectées`,
        severity: 'critical' as const,
        description: 'Intervention immédiate requise pour éviter dégradation service'
      }] : []),
      ...(congestionMetrics.warning > 0 ? [{
        id: 2,
        title: `${congestionMetrics.warning} zones en alerte`,
        severity: 'warning' as const,
        description: 'Surveillance accrue recommandée'
      }] : []),
      ...(alerts && alerts.length > 5 ? [{
        id: 3,
        title: `${alerts.length} alertes actives`,
        severity: 'info' as const,
        description: 'Volume d\'alertes supérieur à la normale'
      }] : [])
    ]
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <ExecutiveKpiCard
            ...
            icon={congestionMetrics.critical > 0 ? '🚨' : '⚠️'}
          />
        </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-purple-900/20 to-purple-900/5 border border-purple-500/20 rounded-2xl p-6 flex flex-col items-center justify-center">
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-slate-500">Basé sur</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 mt-1">
                    {congestionMetrics.critical} zones en état critique nécessitent attention
                  </p>
                </div>
              </div>
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <KpiCard
            title="Charge Réseau Max"
            value={metrics?.peakValue || '0.0'}
            trend={`Nœud ${metrics?.peakId}`}
            color="red"
          />
          <KpiCard
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-slate-800/20 border border-white/5 rounded-2xl p-5">
              <h3 className="text-xs font-black text-slate-400 uppercase mb-4">Métriques Rapides</h3>
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Monitoring système et infrastructure</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-800/20 border-2 border-green-500/30 rounded-2xl p-6">
            ...
            <p className="text-3xl font-black text-green-400">OPÉRATIONNEL</p>
            <p className="text-xs text-slate-400 mt-2">✓ Tous systèmes nominaux</p>
          ...
          <div className="bg-slate-800/20 border border-blue-500/30 rounded-2xl p-6">
            ...
            <div className="space-y-2">
              ...
              <div className="text-[10px] text-slate-500 pt-2 border-t border-white/5">Trend: ↓ -8% vs hier</div>
            </div>
          </div>

          <div className="bg-slate-800/20 border border-purple-500/30 rounded-2xl p-6">
            <h3 className="text-sm font-black text-slate-300 mb-4">Métriques</h3>
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                <span className="text-slate-400">Débit Total</span>
                <span className="font-bold text-blue-400">{metrics?.dataVolume || '0'} Gb/h</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Santé Réseau</span>
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className={`relative bg-gradient-to-br ${congestionMetrics.critical > 0 ? 'from-red-900/40 to-red-900/10' : 'from-green-900/40 to-green-900/10'} border-2 ${congestionMetrics.critical > 0 ? 'border-red-500/60' : 'border-green-500/60'} rounded-2xl p-6 hover:scale-105 transition-transform`}>
            <div className="absolute top-4 right-4 text-3xl animate-pulse">{congestionMetrics.critical > 0 ? '🚨' : '✅'}</div>
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              {congestionMetrics.critical > 0 && <p className="text-xs text-red-300 animate-pulse font-bold">⚠️ Action requise immédiatement</p>}
              {congestionMetrics.critical === 0 && <p className="text-xs text-green-300 font-bold">✓ Aucune alerte critique</p>}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-yellow-900/40 to-yellow-900/10 border-2 border-yellow-500/60 rounded-2xl p-6 hover:scale-105 transition-transform relative">
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              {congestionMetrics.warning > 0 && <p className="text-xs text-yellow-300 font-bold">📊 À monitorer étroitement</p>}
              {congestionMetrics.warning === 0 && <p className="text-xs text-green-300 font-bold">✓ Zones stables</p>}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-900/40 to-green-900/10 border-2 border-green-500/60 rounded-2xl p-6 hover:scale-105 transition-transform relative">
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              <p className="text-xs text-green-300 font-bold">✓ {Math.round((((stats?.totalCells || 0) - congestionMetrics.critical - congestionMetrics.warning) / Math.max(1, stats?.totalCells || 1)) * 100)}% Normales</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 relative bg-slate-900/50 backdrop-blur-sm border border-white/5 rounded-2xl overflow-hidden shadow-xl min-h-[600px]">
            <div className="absolute top-6 left-6 z-10 bg-slate-950/80 backdrop-blur-lg px-4 py-3 rounded-xl border border-white/10">
              <p className="text-xs font-black text-red-400 uppercase animate-pulse">🔴 En direct</p>
              <p className="text-xs text-slate-300 font-bold mt-1">Cliquez sur les cellules pour détails</p>
            </div>
            <CongestionMap
              congestionCells={congestionCells || []}
              warnThreshold={70}
    ...
    const recommendations = [
      {
        ...
        description: `La charge actuelle de ${metrics?.peakValue} Mbps dépasse le seuil critique de 100 Mbps. Risque de dégradation service.`,
        impact: 'Rééquilibrage immédiat peut améliorer les performances de 25%',
        action: 'Appliquer Rééquilibrage',
        priority: 9
      },
      ...(congestionMetrics.critical > 0 ? [{
        id: '2',
        type: 'warning' as const,
        title: 'Zones multiples en état critique',
        description: `${congestionMetrics.critical} cellules dépassent 90% de charge. Surveillance accrue nécessaire.`,
        impact: 'Prévention des pannes potentielles affectant ${congestionMetrics.critical * 1000} utilisateurs',
        action: 'Voir Détails',
        priority: 8
      }] : []),
      {
        ...
        description: 'Analyse ML détecte une opportunité de redistribution du trafic vers les zones sous-utilisées.',
        impact: 'Amélioration globale de 15% du débit réseau',
      ...
      },
      {
        id: '4',
        type: 'info' as const,
        title: 'Pattern de charge hebdomadaire identifié',
        description: 'Les pics se produisent systématiquement entre 18h-20h en semaine.',
        impact: 'Planification proactive de la capacité',
        priority: 4
      }
    ]
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Analyse avancée et optimisation réseau</p>
          </div>
          <DateRangePicker
            onChange={(start, end) => {
        ...
        <div className="bg-slate-800/20 border border-white/5 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Télécharger les données et visualisations</p>
            </div>
            <ExportButton
  ...
  if (role === 'operations_manager') {
    ...
    const insights = [
      {
        ...
        text: `Le réseau affiche une performance exceptionnelle avec un uptime de 99.8% sur les 7 derniers jours, dépassant légèrement l'objectif SLA de 99.9%.`,
        confidence: 0.95,
        dataPoints: ['Uptime: 99.8%', 'Objectif: 99.9%', 'Période: 7j']
      },
      {
        id: '2',
        category: 'trend' as const,
        text: `Tendance positive sur la résolution d'incidents : le MTTR a diminué de 15% pour atteindre 12 minutes, en avance sur l'objectif de 15 minutes.`,
        confidence: 0.89,
        dataPoints: ['MTTR actuel: 12min', 'Cible: 15min', 'Amélioration: -15%']
      },
      {
        id: '3',
        category: 'capacity' as const,
        text: `La capacité réseau est bien dimensionnée avec une charge moyenne de ${metrics?.avgLoad || '0'} Erlang, permettant une marge de manœuvre confortable.`,
        confidence: 0.82,
    ...
    const actionItems = [
      {
        ...
        description: `La cellule atteint ${metrics?.peakValue} Mbps, proche de la limite de capacité`,
        assignee: 'Équipe Performance',
      ...
      },
      {
        ...
        title: 'Documentation mise à jour KPIs',
        description: 'Mettre à jour les seuils d\'alerte dans la documentation',
        assignee: 'Équipe Qualité',
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black text-white">Rapport Opérations</h2>
            <p className="text-sm text-slate-400 mt-1">Vue exécutive et reporting</p>
          </div>
          <div className="flex items-center gap-4">
            <PeriodComparator
        ...
        <div className="bg-slate-800/20 border border-white/5 rounded-2xl p-6">
          ...
          <div className="mt-4 pt-4 border-t border-white/5">
            <p className="text-[10px] text-slate-500">📊 Moy: 82% • Pic: 95% • Min: 58%</p>
          </div>
        </div>
        )}
      </div>
